...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['no_invoice'])) {
    ...
    $stmt->bind_param("sdddsis", $tanggal, $total_harga, $pajak, $total, $id_karyawan, $no_rekening_toko, $no_invoice);
    $stmt->execute();


    $queryProduk = $koneksi->prepare("SELECT Kode_Produk, Kuantitas_Produk, Sub_Total_Kuantitas_Tiap_Produk FROM detail_transaksi WHERE NO_Invoice = ?");
...
<body>
    ...
    <table>
        ...
        <div class="pricing">
            <p>Sub Total: Rp<?= number_format($total_harga, 2, ',', '.') ?></p>
            <p>Pajak (5%): Rp<?= number_format($pajak, 2, ',', '.') ?></p>
            <p class="total">Total: Rp<?= number_format($total, 2, ',', '.') ?></p>
        </div>
