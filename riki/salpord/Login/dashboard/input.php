...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $kode_produk = $_POST['Kode_Produk']; // Produk yang dipilih
    $quantity = isset($_POST['quantity'][$kode_produk]) ? intval($_POST['quantity'][$kode_produk]) : 0; // Jumlah produk
    ...
    if ($quantity > 0) {
        ...
        if (isset($harga_produk)) {
            ...
            $insert_stmt = $koneksi->prepare("INSERT INTO detail_transaksi (NO_Invoice, Kode_Produk, Kuantitas_Produk, Sub_Total_Kuantitas_Tiap_Produk) VALUES ('0', ?, ?, ?)");
            $insert_stmt->bind_param("sii", $kode_produk, $quantity, $sub_total);
