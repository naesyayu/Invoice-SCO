...
<body>
    ...
    $result = $koneksi->query("SELECT DISTINCT MONTH(Tanggal_Invoice) AS bulan FROM invoice WHERE YEAR(Tanggal_Invoice) = $tahun");
    if ($result->num_rows === 0 || empty($_GET['tahun'])) {
        ...
        foreach ($result as $row) {
            ...
            $bulan = $row['bulan'];
            $month = date('F', mktime(0, 0, 0, $bulan, 1));
            if ($tahun != 0) {
                $result1 = $koneksi->query("SELECT NO_Invoice, Total_Harga FROM invoice WHERE YEAR(Tanggal_Invoice) = $tahun AND MONTH(Tanggal_Invoice) = $bulan");
    ...
    ?>
        <tr>
            <td><?= $month ?>, <?= $tahun ?></td>
            <td style="width: 100px;"><button class="lihat" onclick="window.location.href='rekapitulasiharian.php?tahun=<?= $tahun ?>&bulan=<?= $bulan ?>'">Lihat</button></td>
